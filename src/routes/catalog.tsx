  ...
  head: () => ({
    meta: [
      { title: "Catalog — Ledgerline ERP, POS & Restaurant Licenses" },
      {
        name: "description",
        content:
          "Every Ledgerline edition: ERP, POS and restaurant management licenses with prices.",
      },
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <p className="mt-2 max-w-[52ch] text-subtle">
        Every edition is a digital license. Your order is confirmed after secure payment.
      </p>

      {error ? (
        <div role="alert" className="mt-8 text-sm">
          <p>{error.message}</p>
          <button className="mt-3 text-brand" onClick={() => void refetch()}>
